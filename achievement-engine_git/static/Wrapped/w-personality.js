...
const stats = s.stats || {};
const webWordBinge = document.getElementById('web-word-binge');
const webWordStreak = document.getElementById('web-word-streak');
const webWordStreakSide = document.getElementById('web-word-streak-side');
const webBingeCount = Number(stats.bingeSessionCount || 0);
const webStreakDays = Number(stats.longestStreak || 0);
if (webWordBinge) webWordBinge.textContent = `BINGE ${webBingeCount.toLocaleString()}`;
if (webWordStreak) webWordStreak.textContent = `STREAK ${webStreakDays.toLocaleString()}`;
if (webWordStreakSide) webWordStreakSide.textContent = `${webStreakDays.toLocaleString()} DAY STREAK`;

// Set personality class name
const personalityEl = document.getElementById('s6-l2');
if (personalityEl && stats.personality) {
  personalityEl.innerHTML = `<center>${stats.personality.name}</center>`;
}

// Reset text
for (let i = 1; i <= 4; i++) {
  const el = document.getElementById(`s6-l${i}`);
  if (el) el.classList.remove('text-visible');
}

setTimeout(() => document.getElementById('s6-l1').classList.add('text-visible'), 400);
setTimeout(() => document.getElementById('s6-l2').classList.add('text-visible'), 1000);
setTimeout(() => {
  document.getElementById('s6-l3').classList.add('text-visible');

  const totalStatChips = (s.userSheet || {}).total_stats || {};
  const mag = Number(totalStatChips.mag || 0);
  const bingeCount = Number(stats.bingeSessionCount || 0);
  const streakDays = Number(stats.longestStreak || 0);

  const poisonDmg = Math.min(25000, (mag * 15) + (bingeCount * 1000));
  const healAmt = (mag * 2) + (streakDays * 180);

  const bingeEl = document.getElementById('s6-binge-count');
  const streakEl = document.getElementById('s6-streak-days');
  const poisonPreviewEl = document.getElementById('s6-poison-preview');
  const healPreviewEl = document.getElementById('s6-heal-preview');
  if (bingeEl) bingeEl.textContent = bingeCount.toLocaleString();
  if (streakEl) streakEl.textContent = `${streakDays.toLocaleString()} days`;
  if (poisonPreviewEl) poisonPreviewEl.textContent = Math.round(poisonDmg).toLocaleString();
  if (healPreviewEl) healPreviewEl.textContent = Math.round(healAmt).toLocaleString();

  setTimeout(() => {
    document.getElementById('s6-l4').classList.add('text-visible');
  }, 500);

  setTimeout(() => {
    spawnDamageText(`+${Math.round(healAmt).toLocaleString()} HP [LEECH]`, 'user', '#00ff88');
    updateHP('user', Math.min(hp.userMaxHP, hp.userCurrentHP + healAmt));

    setTimeout(() => {
      spawnDamageText(`-${Math.round(poisonDmg).toLocaleString()} [POISON]`, 'boss', '#3dff3d');
      triggerShake('light');
      updateHP('boss', getState().bossCurrentHP - poisonDmg);
    }, 900);
  }, 700);
}, 2000);

setTimeout(() => unlockSlide('personality'), 5600);

// ── VERLET JS SPIDER PHYSICS ENGINE ──────────────────────────────────────────
...
window.addEventListener('load', function() {
  ...
  spiderweb.drawParticles = function(ctx, composite) {
    ...
    for (i in composite.particles) {
      ...
      ctx.beginPath(); ctx.arc(point.pos.x, point.pos.y, 1.3, 0, 2*Math.PI);
      ctx.fillStyle = '#7AA'; ctx.fill();
    }
  };

  spider.drawConstraints = function(ctx, composite) {
    var i;
    ctx.beginPath(); ctx.arc(spider.head.pos.x, spider.head.pos.y, 4, 0, 2*Math.PI); ctx.fillStyle=getColor(1); ctx.fill();
    ...
    for (i=3; i<composite.constraints.length; ++i) {
      ...
      if (constraint instanceof DistanceConstraint) {
        ctx.beginPath(); ctx.moveTo(constraint.a.pos.x, constraint.a.pos.y); ctx.lineTo(constraint.b.pos.x, constraint.b.pos.y);
        if((i>=2&&i<=4)||(i>=(2*9)+1&&i<=(2*9)+2)||(i>=(2*17)+1&&i<=(2*17)+2)||(i>=(2*25)+1&&i<=(2*25)+2)){ctx.save();constraint.draw(ctx);ctx.strokeStyle=getColor(2);ctx.lineWidth=3;ctx.stroke();ctx.restore();}
        else if((i>=4&&i<=6)||(i>=(2*9)+3&&i<=(2*9)+4)||(i>=(2*17)+3&&i<=(2*17)+4)||(i>=(2*25)+3&&i<=(2*25)+4)){ctx.save();constraint.draw(ctx);ctx.strokeStyle=getColor(3);ctx.lineWidth=2;ctx.stroke();ctx.restore();}
        else if((i>=6&&i<=8)||(i>=(2*9)+5&&i<=(2*9)+6)||(i>=(2*17)+5&&i<=(2*17)+6)||(i>=(2*25)+5&&i<=(2*25)+6)){ctx.save();ctx.strokeStyle=getColor(4);ctx.lineWidth=1.5;ctx.stroke();ctx.restore();}
        else{ctx.strokeStyle=getColor(5);ctx.stroke();}
      }
    }
  };
  ...
  var loop = function() {
    ti++;
    if (Math.floor(Math.random()*4) === 0) sim.crawl(((legIndex++)*3)%8);
    sim.frame(16); sim.draw();
